...
router.post('/driver-online',async(req,res)=>{try{const userId=await authUser(req);if(!userId)return res.status(401).json({error:'Authentication required'});
 const online=req.body?.online===true;if(online)await MarketAvailabilityService.requireCapability({countryCode:req.body?.countryCode,marketCity:req.body?.marketCity,zoneId:req.body?.zoneId,capability:'driver_online',endpoint:'/api/markets/driver-online'});
