...
router.get('/status', async (req, res) => {
  ...
  try {
    ...
    const { data: requestRows, error: requestError } = await supabaseAdmin.from('driver_onboarding_requests').select('id,requirement_code,request_type,item,public_message,status,sent_at,resolved_at,updated_at,permission_consumed_at').eq('driver_id',driverId).order('created_at',{ascending:false});
...
router.put('/profile',async(req,res)=>{const driverId=await authenticatedDriver(req,res);if(!driverId)return;try{
  ...
  if(dateOfBirthPresent){const{data:identityProfile,error:identityError}=await supabaseAdmin.from('profiles').select('date_of_birth,onboarding_completed,verification_status,driver_review_status').eq('id',driverId).single();if(identityError||!identityProfile)throw identityError||new Error('Driver profile not found.');const{data:correctionRows,error:correctionError}=await supabaseAdmin.from('driver_onboarding_requests').select('status,request_type,permission_consumed_at').eq('driver_id',driverId).eq('request_type','identity_correction');if(correctionError)throw correctionError;const editability=DriverIdentityEditabilityService.resolve(identityProfile,correctionRows||[]);if(!editability.dateOfBirthEditable)return res.status(403).json({code:'DOB_CHANGE_NOT_ALLOWED',message:'Date of birth cannot be changed after verification has started.'});previousDateOfBirth=identityProfile.date_of_birth||null;}
