...
/**
 * Calculate price with surge and DB-driven pricing rules
 */
router.post('/calculate-price', async (req: Request, res: Response) => {
  try {
    const { lat, lng, basePrice, distanceKm, serviceType, countryCode, currencyCode } = req.body;

    if (lat === undefined || lng === undefined) {
      return res.status(400).json({ error: 'lat and lng are required' });
    }

    // Backward compatibility: if neither basePrice nor distanceKm is provided, we can't calculate
    ...
    // Resolve base price using new precedence logic
    ...
    // Calculate surge based on real-time demand/supply
