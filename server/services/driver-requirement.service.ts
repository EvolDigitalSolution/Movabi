...
export interface DriverVehicleValidation {
  vehicle_class?: string|null; capacity?: string|null; type?: string|null; make?: string|null; model?: string|null;
  color?: string|null; colour?: string|null; year?: number|string|null; license_plate?: string|null;
  registration_number?: string|null; service_eligibility?: string[]|string|null; bicycle_type?: string|null;
}
...
export class DriverRequirementService {
  static resolve(input: { profile: Record<string, any>; vehicle?: DriverVehicleValidation|null; authEmailConfirmed: boolean; adminRequests?: DriverAdminRequest[]; countryCode?: string|null; marketCity?: string|null; now?: Date }): DriverRequirementResolution {
    const profile=input.profile||{}, vehicle=input.vehicle||{}, now=input.now||new Date();
    ...
    if(selectedServices.length){
      ...
      if(vehicleBased){
        ...
        add('vehicle.colour','Vehicle colour','vehicle',this.has(vehicle.color||vehicle.colour),'Add the vehicle colour.'); add('vehicle.year','Vehicle year','vehicle',Number(vehicle.year)>1900,'Add a valid vehicle year.');
        add('vehicle.registration','Vehicle registration','vehicle',this.has(vehicle.license_plate||vehicle.registration_number),'Add the registration number.');
        ...
        add('vehicle.bicycle_declaration','Bicycle details','vehicle',this.has(vehicle.bicycle_type||vehicle.type)||profile.bicycle_declaration===true,'Add bicycle details or confirm your bicycle declaration.');
    ...
    const openAdmin=adminRequests.filter(r=>r.status!=='approved'); const review=profile.verification_status==='under_review'||profile.driver_review_status==='under_review'; const approved=profile.is_verified===true||profile.verification_status==='approved'; const paused=['paused','suspended','blocked'].includes(String(profile.account_status||'').toLowerCase());
    const overallStatus=paused?'paused':approved?'approved':openAdmin.length||requirements.some(r=>r.status==='invalid'||r.status==='rejected')?'action_required':review?'under_review':!selectedServices.length?'not_started':blockers.length?'incomplete':'ready_to_submit';
  ...
  private static services(profile:Record<string,any>,vehicle:DriverVehicleValidation):CanonicalDriverService[]{const raw=profile.driver_service_types||profile.verification_items?.driver_service_types||vehicle.service_eligibility||[];const values=Array.isArray(raw)?raw:String(raw||'').replace(/[\[\]"]/g,'').split(',');return Array.from(new Set(values.map(v=>String(v).trim().toLowerCase()).map(v=>['van','moving','van_moving'].includes(v)?'van-moving':v).filter((v):v is CanonicalDriverService=>['ride','delivery','errand','van-moving'].includes(v))));}
  private static vehicle(vehicle:DriverVehicleValidation,profile:Record<string,any>):DriverOperatingVehicle{const value=String(vehicle.vehicle_class||vehicle.capacity||vehicle.type||profile.operating_vehicle||'').toLowerCase();if(!value)return null;if(/bicycle|bike|cycle/.test(value))return'bicycle';if(/motorcycle|motorbike|moped|scooter/.test(value))return'motorcycle';if(/large.?van|luton|box.?van/.test(value))return'large_van';if(/small.?van|\bvan\b/.test(value))return'small_van';return'car';}
