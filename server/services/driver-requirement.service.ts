...
export class DriverRequirementService {
  static resolve(input: { profile: Record<string, any>; vehicle?: Record<string, any>|null; authEmailConfirmed: boolean; adminRequests?: DriverAdminRequest[]; countryCode?: string|null; marketCity?: string|null; now?: Date }): DriverRequirementResolution {
  ...
  private static services(profile:Record<string,any>,vehicle:Record<string,any>):CanonicalDriverService[]{const raw=profile.driver_service_types||profile.verification_items?.driver_service_types||vehicle.service_eligibility||[];const values=Array.isArray(raw)?raw:String(raw||'').replace(/[\[\]"]/g,'').split(',');return Array.from(new Set(values.map(v=>String(v).trim().toLowerCase()).map(v=>['van','moving','van_moving'].includes(v)?'van-moving':v).filter((v):v is CanonicalDriverService=>['ride','delivery','errand','van-moving'].includes(v))));}
  private static vehicle(vehicle:Record<string,any>,profile:Record<string,any>):DriverOperatingVehicle{const value=String(vehicle.vehicle_class||vehicle.capacity||vehicle.type||profile.operating_vehicle||'').toLowerCase();if(!value)return null;if(/bicycle|bike|cycle/.test(value))return'bicycle';if(/motorcycle|motorbike|moped|scooter/.test(value))return'motorcycle';if(/large.?van|luton|box.?van/.test(value))return'large_van';if(/small.?van|\bvan\b/.test(value))return'small_van';return'car';}
