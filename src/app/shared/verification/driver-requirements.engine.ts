...
export type DriverRequirementStatus =
    | 'missing'
    | 'uploaded'
    | 'under_review'
    | 'approved'
...
export function normaliseVehicleClass(vehicle?: any): DriverVehicleClass {
    ...
    if (raw.includes('bike') || raw.includes('bicycle') || raw.includes('cycle') || raw.includes('scooter') || raw.includes('moped')) {
        return 'bike';
    }

    if (raw.includes('large_van') || raw.includes('large van') || raw.includes('luton') || raw.includes('box van')) {
        return 'large_van';
    }

    if (raw.includes('small_van') || raw.includes('small van') || raw === 'van' || raw.includes('medium van')) {
        return 'small_van';
    }

    if (raw.includes('xl') || raw.includes('7') || raw.includes('seven') || raw.includes('minibus')) {
        return 'xl_7_seater';
    }
...
export function getDriverRequirements(input: DriverRequirementsInput): DriverRequirementResult[] {
    ...
    if (country.adminWarning) {
        requirements.push(result('country_config', 'Country requirements', country.adminWarning, 'uploaded', 'warning', 'country', false));
    }

    if (!selectedServices.length) {
        requirements.push(result('selected_services', 'Services', 'Select at least one service type before review.', 'missing', 'blocker', 'service', true));
    }

    selectedServices
        .filter((service) => !ALLOWED_SERVICES[vehicleClass].includes(service))
        .forEach((service) => requirements.push(result(
            `service_${service}_not_allowed`,
            'Service eligibility',
            `${serviceLabel(service)} is not available for ${vehicleClassLabel(vehicleClass)}.`,
            'missing',
            'blocker',
            'service',
            true,
    ...
    if (selectedServices.some((service) => service === 'errand' || service === 'delivery')) {
        ...
        requirements.push(documentRequirement(insuranceKeysObject(documents), insuranceKeys, ['courier_insurance_status', 'insurance_status'], 'courier_insurance', 'Courier insurance', 'Upload correct insurance before accepting this job type.'));
    ...
    requirements.push(result(
        'stripe_connect',
        'Stripe Connect',
        stripeReady ? 'Stripe payout setup is connected.' : 'Stripe payout setup is missing.',
        stripeReady ? 'approved' : 'missing',
        stripeReady ? 'warning' : 'blocker',
        'payout',
        true
    ));
...
export function getBlockingRequirements(input: DriverRequirementsInput): DriverRequirementResult[] {
    return getDriverRequirements(input).filter((requirement) =>
...
function documentRequirement(
    ...
    if (!hasFile) {
        return result(key, label, missingMessage, 'missing', 'blocker', 'document', true, serviceTypes);
    }

    if (status === 'rejected' || status === 'expired') {
        return result(key, label, `${label} is ${status}.`, status, 'blocker', 'document', true, serviceTypes);
    }

    if (status === 'under_review') {
        return result(key, label, `${label} is under admin review.`, 'under_review', 'review', 'document', true, serviceTypes);
    }
...
function insuranceKeysObject(source: any): any {
    return source || {};
}
