...
export class AdminService {
  ...
  async fetchStats() {
    ...
    const activeJobs = (jobs || []).filter((j: any) =>
      ['accepted', 'arrived', 'assigned', 'in_progress', 'heading_to_pickup'].includes(j.status)
    ).length;
  ...
  async createSubscriptionPlan(payload: any) {
    const cleanPayload = this.cleanSubscriptionPlanPayload(payload, false);
    ...
    if (error) {
      console.error('[AdminService] createSubscriptionPlan error:', error, cleanPayload);
      throw new Error(error.message);
    }
  ...
  async updateSubscriptionPlan(id: string, payload: any) {
    const cleanPayload = this.cleanSubscriptionPlanPayload(payload, true);
    ...
    if (error) {
      console.error('[AdminService] updateSubscriptionPlan error:', error, cleanPayload);
      throw new Error(error.message);
    }
  ...
  private cleanSubscriptionPlanPayload(payload: any, isUpdate: boolean) {
    const name = String(payload?.name || '').trim();
    const planCode = String(payload?.plan_code || name || 'plan')
      .trim()
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, '_')
      .replace(/^_+|_+$/g, '');
    ...
    const currencyCode = String(payload?.currency_code || 'GBP').trim().toUpperCase();
    const countryCode = String(payload?.country_code || 'GB').trim().toUpperCase();

    const features = Array.isArray(payload?.features)
      ? payload.features.map((x: unknown) => String(x).trim()).filter(Boolean)
      : String(payload?.features || payload?.featuresText || '')
          .split('\n')
          .map((x: string) => x.trim())
          .filter(Boolean);
    ...
    const cleanPayload: any = {
      name,
      display_name: String(payload?.display_name || name || planCode || 'Subscription Plan').trim(),
      plan_code: planCode,
      description: String(payload?.description || '').trim(),
      price,
      amount: Number(payload?.amount ?? price),
      interval: String(payload?.interval || 'month').trim(),
      features,
      country_code: countryCode,
      currency_code: currencyCode,
      currency_symbol: String(payload?.currency_symbol || this.symbolFromCode(currencyCode)).trim(),
      stripe_price_id: String(payload?.stripe_price_id || `price_manual_${planCode || Date.now()}`).trim(),
  ...
  private symbolFromCode(code?: string | null): string {
