...
import {CANONICAL_DRIVER_PROFILE_SELECT,mapDriverProfile,parseResidentialAddress} from '../../server/models/driver-profile.model';
...
describe('canonical residential address',()=>{
 ...
 it('does not substitute vehicle or legacy fields for the canonical address',()=>expect(mapDriverProfile({...row,current_address:null},true).residentialAddress).toBeNull());
 ...
 it('persists current_address with the schema-safe projection and verifies the returned row',()=>{expect(route).toContain("profileUpdates={current_address:residentialAddress");expect(route).toContain('select(CANONICAL_DRIVER_PROFILE_SELECT).single()');expect(route).toContain("if(!profile.residentialAddress)throw");});
 it('selects only canonical profile columns after update',()=>{expect(CANONICAL_DRIVER_PROFILE_SELECT).toBe('id,full_name,phone,date_of_birth,current_address,verification_status,updated_at');expect(CANONICAL_DRIVER_PROFILE_SELECT).not.toContain('bicycle_declaration');});
 it('does not write declarations as physical profile columns',()=>{const put=route.slice(route.indexOf("router.put('/profile'"),route.indexOf("router.get('/vehicle'"));expect(put).not.toMatch(/profileUpdates=.*bicycle_declaration/);expect(put).toContain('verification_items:verificationItems');});
 it('preserves bicycle declarations in onboarding verification data',()=>{const resolved=DriverRequirementService.resolve({profile:{...row,driver_service_types:['delivery'],verification_items:{bicycle_declaration:true,delivery_equipment_confirmed:true}},vehicle:{id:'bike',userId:'driver-1',type:'bike',make:null,model:null,colour:null,year:null,registrationNumber:null,capacity:'bike',serviceEligibility:['delivery'],status:'saved'},authEmailConfirmed:true});expect(resolved.automaticRequirements.find(item=>item.code==='vehicle.bicycle_declaration')?.completed).toBe(true);});
 it('surfaces PostgREST schema errors with their original code',()=>{expect(route).toContain("code:details.code||'PROFILE_SAVE_FAILED'");expect(route).toContain("code:details.code||'PROFILE_SAVE_FAILED'}");});
